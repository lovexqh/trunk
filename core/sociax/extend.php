...
function ubb($Text) {
	...
	// $Text=preg_replace("/\[p=(.+?)\](.+?)\[\/p\]/is", "<p
	// class=\"\\1\">\\2</p>", $Text );
	// $Text=preg_replace("/\[span=(.+?)\](.?)\[\/span\]/is", "<span
	// class=\"\\1\">\\2</span>", $Text );
...
function sortByCol($array, $keyname, $dir = SORT_ASC) {
	return sortByMultiCols ( $array, array (
			$keyname => $dir 
	) );
}
...
/**
 * Format a mySQL string correctly for safe mySQL insert (no mater if magic
 * quotes are on or not)
 */
function escape($str) {
...
function group_themeformat($data) {
	$res = "<a href=" . U ( 'group/Group/search', array (
			'gid' => $_SESSION ['_group_weibo_format'],
			'k' => urlencode ( $data [1] ) 
	) ) . ">" . $data [0] . "</a>";
...
function isLegalUsername($username) {
	...
	// $username)
	return preg_match ( "/^[\x{2e80}-\x{9fff}A-Za-z0-9_-]+$/u", $username ) && mb_strlen ( $username, 'UTF-8' ) >= 2 && mb_strlen ( $username, 'UTF-8' ) <= 10;
}
 ...
 * @param string $flag
 *        	Where the cache contents are grouped
 * @return bool mixed on failure to retrieve contents or the cache
 *         contents on success
 */
 ...
 * this function instructs the backend to reset those keys and perform any
 * cleanup since blog or site IDs have changed since cache init.
 *
 * @since 2.6.0
 *       
...
function getChildTree($modelName, $nodeId = 0, $cur_level = 1, $treeList = array (), $nodeList) {
	$separator = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	if (empty ( $nodeList )) {
		if (! isset ( $modelName ))
			return;
	...
	foreach ( $nodeList as $key => $node ) {
		if ($node ['pid'] == $nodeId) {
			...
			$node ['is_leaf'] = ! hasChild ( $nodeList, $node );
			$treeList [] = $node;
			if (! $node ['is_leaf']) {
				getChildTree ( $modelName, $node ['id'], $cur_level + 1, &$treeList, $nodeList );
			}
		}
	}
	return $treeList;
