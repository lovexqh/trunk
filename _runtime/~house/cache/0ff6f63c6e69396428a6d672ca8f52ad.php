...
<div class="container solid1">
	...
	<div class="content">
		<form action="<?php echo U('house/homeRegion/saveOrUpdate');?>" method="post"
			enctype="multipart/form-data">
			<table class="myaddhome">
				<tr class="graybg">
					<td class="textr" width="10%"><span class="fontw">名称：</span></td>
					<td class="textl" colspan="7"><input maxlength="40"
						type="text" name="name" value="<?php echo ($vo["name"]); ?>" class="myinput" id="f1">
					</td>
				</tr>
				<tr class="whitebg">
					 <td class="textr" width="10%">
                        <span class="fontw">上级小区：</span>
                    </td>
                    <td class="textl"  colspan="7">
                     <select name="pid">
			         <option value="0" <?php if($vo["pid"] ==  0): ?>selected="selected"<?php endif; ?>>首页</option>
			    		   <?php if(is_array($hrList)): ?><?php $i = 0;?><?php $__LIST__ = $hrList?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$hr): ?><?php ++$i;?><?php $mod = ($i % 2 )?><option value="<?php echo ($hr['id']); ?>" <?php if($vo['pid'] == $hr['id']): ?>selected="selected"<?php endif; ?>>
							       <?php echo ($hr["name"]); ?></option><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
       				 </select></td>
					 
				</tr>
				<tr class="graybg" height="145px">
					<td class="textr"><span class="fontw">户型图上传：</span></td>
					<td class="textl" colspan="7"></td>
				</tr>
				...
				<tr class="graybg" height="165px">
					...
					<td class="textl" colspan="7"><textarea cols="82" rows="10"
							id="f2" name="remark"><?php echo ($vo["remark"]); ?></textarea></td>
			...
			<input type="hidden" id="upimgs" name="timage" value="<?php echo ($vo["timage"]); ?>"> <input
				type="hidden" id="upimgs1" name="oimage" value="<?php echo ($vo["oimage"]); ?>"> <input
				type="hidden" name="id" value="<?php echo ($vo["id"]); ?>" />
		</form>
	</div>
</div>
