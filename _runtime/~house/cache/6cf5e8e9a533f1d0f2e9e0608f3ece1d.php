<?php if (!defined('THINK_PATH')) exit();?><script type="text/javascript" src="__PUBLIC__/js/admin.js" defer="defer"></script>
...
<div class="container solid1">
	...
	<div class="content">
		...
		<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$content): ?><?php ++$i;?><?php $mod = ($i % 2 )?><tr <?php if(($mod)  ==  "0"): ?>class="graybg"<?php endif; ?><?php if(($mod)  ==  "1"): ?>class="whitebg"<?php endif; ?> ><td><input type="checkbox" value="<?php echo ($content["id"]); ?>" /></td><td><?php echo ($content["username"]); ?></td><td><?php echo ($content["remark"]); ?></td><td><?php echo (toDate($content["createtime"],'Y-m-d H#i#s')); ?></td>
		<td class="managehometd"><a href="<?php echo U('house/HomeHistory/read',array('id'=>$content['id']));?>"><img src="__PUBLIC__/admin/images/showhome.gif"/>查看</a> | <a href="<?php echo U('house/HomeHistory/edit',array('id'=>$content['id']));?>"><img src="__PUBLIC__/admin/images/updatehome.gif"/>修改</a>
