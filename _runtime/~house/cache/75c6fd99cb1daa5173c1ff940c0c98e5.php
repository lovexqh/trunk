...
<div class="container solid1">
	...
	<p class="addbtn"><a href="<?php echo U('house/homeRegion/add',array('pid'=>0));?>"><span class="addbtnbg">添加小区</span></a>	<input type="button" onclick="allSelect()" class="searchbut" value="全 选">&nbsp;
	...
	<div class="content">
		...
		<?php if(is_array($list)): ?><?php $i = 0;?><?php $__LIST__ = $list?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$content): ?><?php ++$i;?><?php $mod = ($i % 2 )?><tr <?php if(($mod)  ==  "0"): ?>class="graybg"<?php endif; ?><?php if(($mod)  ==  "1"): ?>class="whitebg"<?php endif; ?> ><td><input type="checkbox" value="<?php echo ($content["id"]); ?>" /></td><td style="text-align: left;padding-left: 20px;"><?php echo ($content["name"]); ?></td><td><?php echo (toDate($content["createtime"],'Y-m-d H#i#s')); ?></td>
		<td class="managehometd"><a href="<?php echo U('house/homeRegion/add',array('pid'=>$content['id']));?>"><img src="__PUBLIC__/admin/images/addbtn.gif"/>添加</a> | <a href="<?php echo U('house/homeRegion/edit',array('id'=>$content['id']));?>"><img src="__PUBLIC__/admin/images/updatehome.gif"/>修改</a>
