...
class UserModel extends Model {
    ...
    public function getUserList($map = '', $show_dept = false, $show_user_group = false, $field = '*', $order = 'id ASC', $limit = 30) {
    	...
    	$ids = getSubByKey($res['data'], 'id');
    	//部门信息
    	if ($show_dept) {
    	}
    	//用户组
    	if ($show_user_group) {
    		$temp_user_group = model('UserGroup')->getUserGroupByid($ids);
    		//转换成array($id => $user_group)的格式
    		$user_group = array();
    ...
    public function deleteUser($ids) {
    	...
    	foreach($ids as $k => $v) {
    		if (!is_numeric($v) || $v == $GLOBALS['ts']['user']['id']) {
    			unset($ids[$k]);
    		}
    	}
		...
		if ($res) {
			...
			M('invite_record')->where(array('id'=>array('IN',$ids),'fid'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			M('login')->where($map)->delete();
			M('login_record')->where($map)->delete();
			model('Message')->where(array('from_id'=>array('IN',$ids),'to_id'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			M('notify')->where(array('from'=>array('IN',$ids),'receive'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			...
			M('friend')->where(array('id'=>array('IN',$ids),'friend_id'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			M('gift_user')->where(array('fromUserId'=>array('IN',$ids),'toUserId'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			M('group_invite_verify')->where($map)->delete();
			M('group_member')->where($map)->delete();
			M('group_photo')->where(array('userId'=>array('IN',$ids)))->delete();
			...
			M('login')->where($map)->delete();
			M('login_record')->where($map)->delete();
			M('photo')->where(array('userId'=>array('IN',$ids)))->delete();
			M('photo_album')->where(array('userId'=>array('IN',$ids)))->delete();
			M('photo_index')->where(array('userId'=>array('IN',$ids)))->delete();
			...
			D('Follow', 'weibo')->where(array('id'=>array('IN',$ids),'fid'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			D('FollowGroup', 'weibo')->where(array('id'=>array('IN',$ids),'follow_id'=>array('IN',$ids),'_logic'=>'OR'))->delete();
			M('follow_group_link')->where($map)->delete();
			D('Star', 'weibo')->where($map)->delete();
			//删除用户附件
			$all_attach	=	M('Attach')->where(array('userId'=>array('IN',$ids)))->findAll();
			foreach($all_attach as $v){
				unlink(UPLOAD_PATH.'/'.$v['savepath'].$v['savename']);
			}
			$result = M('attach')->where(array('userId'=>array('IN',$ids)))->delete();
			if($result){
	...
	private function upbase( ){
		...
		if( M('user')->where("name='{$nickname}' AND id!={$this->id}")->find() ){
	    ...
	    M('user')->where("id={$this->id}")->data($data)->save();
	    //修改登录用户缓存信息--名称
	    $userLoginInfo = S('S_userInfo_'.$this->id);
	    if(!empty($userLoginInfo)) {
	    	$userLoginInfo['name'] = $data['name'];
	    	S('S_userInfo_'.$this->id, $userLoginInfo);
	    }
	    $_SESSION['userInfo'] = D('User', 'home')->find($this->id);
	...
	/**
	 * 根据标示符(id或name或email或domain)获取用户信息
	 *
	 * 首先检查缓存(缓存ID: user_用户id / user_用户name), 然后查询数据库(并设置缓存).
	 *
	 * @param string|int $identifier      标示符内容
	 * @param string     $identifier_type 标示符类型. (id, name, email, domain之一)
	 */
	public function getUserByIdentifier($identifier, $identifier_type = 'name')
	{
     ...
     * 缓存用户列表
     *
     * 缓存key的格式为: user_用户id 和 user_用户昵称
     *
    ...
    public function isEmailAvailable($email,$id)
    {
        if (!isValidEmail($email)) // Email格式错误
            ...
            if($id){
                if($res['id'] != intval($id)) return false;
            }else{
                return false;
            }
    ...
    {
        if (!isValidname($name)) // 格式错误
            return false;
        else if ($this->getUserByIdentifier($name, 'name')) // 在TS已存在
            return false;
        else if (UC_SYNC && uc_user_checkname($name) < 0) // 在UCenter已存在或非法
            return false;
        return true;
    }
	/**
	 * 获取用户基本信息缓存
	 *
	 * @param int $id 用户id，string $type 查询类型(id,name,email,domain)
	 * @return array 用户基本信息
	 */
	public function getUserInfoCache($name, $type = 'name') {
		//如果为空，则直接退出
		if(empty($name)) {
			return false;
		}
		if(!in_array($type, array('id','name','email','domain'))) {
			return false;
		}
			
			// 获取用户的id
		if (isset ( self::$nameHash [$type] [$name] )) {
			$id = self::$nameHash [$type] [$name];
		} else {
			// $map[$type] = $defaultValue;
			$id = $this->where ( "{$type}='{$name}'" )->getField ( 'id' );
			// dump($this->getLastSql());
			self::$nameHash [$type] [$name] = $id;
		}

		$userInfo = S('S_userInfo_'.$id);
		//获取用户基本信息缓存
		if(empty($userInfo)) {
			//姓名
			$userInfo = $this->where('id='.$id)->find();
			S('S_userInfo_'.$id, $userInfo);
		}
		return $userInfo;
	}
}
