...
class PassportService extends Service
{
	...
	public function getLocalUser($identifier, $password = null)
	{
		if (empty($identifier))
			return false;
		$user = D('User', 'auth')->getUserByIdentifier($identifier);
		if (!$user) {
	...
	public function isLoggedAdmin() {
		return !empty($_SESSION[C('USER_AUTH_KEY')]);
	}
	...
	public function loginAdmin($identifier, $password) {
		...
		$_SESSION['loginUserName']		=	$user['nickname'];
		$_SESSION['lastLoginTime']		=	$user['last_login_time'];
		$_SESSION['login_count']	=	$user['login_count'];
		$_SESSION [C ( 'ADMIN_AUTH_KEY' )] = $user['name']=="admin";
		service ( 'SystemPopedom' )->saveAccessList ( $user ['id'] );
